...
# ════════════════════════════════════════════
# SELECTORS — diambil langsung dari raw trace
# ════════════════════════════════════════════
# 0x1679c792 = exactInputSingle (versi Zotto, 8-field struct)
# 0xac9650d8 = multicall(bytes[])
# 0x69bc35b2 = unwrapWETH9(uint256,address)
# 0x12210e8a = refundETH()

SEL_EXACT_INPUT  = bytes.fromhex("1679c792")
SEL_UNWRAP       = bytes.fromhex("69bc35b2")
SEL_REFUND_ETH   = bytes.fromhex("12210e8a")
SEL_MULTICALL    = bytes.fromhex("ac9650d8")

# ════════════════════════════════════════════
# ABI minimal (hanya multicall, untuk build_transaction)
# ════════════════════════════════════════════
...
def encode_exact_input_single(token_in, token_out, fee, recipient, deadline, amount_in, amount_out_min=0):
    """
    Encode exactInputSingle dengan selector Zotto (0x1679c792)
    Struct: (address,address,uint24,address,uint256,uint256,uint256,uint160)
    """
    params = encode(
...
def cek_balance():
    ...
    log(f"💰 Balance — ANKR: {ankr_bal:.4f} | USDT: {usdt_bal:.6f}")
...
def swap_ankr_ke_usdt(ankr_amount):
    """
    ANKR native → USDT
    Kirim ETH sebagai value, router wrap jadi WANKR, swap ke USDT
    """
    ...
    swap_data   = encode_exact_input_single(WANKR_TOKEN, USDT_TOKEN, 0, WALLET, deadline, amount_in)
    refund_data = encode_refund_eth()  # kembalikan sisa ETH kalau ada
...
def swap_usdt_ke_ankr():
    """
    USDT → ANKR native
    Sama persis dengan raw trace yang berhasil
    """
    ...
    # exactInputSingle: USDT → WANKR, recipient = 0x0 (router simpan dulu)
    swap_data   = encode_exact_input_single(USDT_TOKEN, WANKR_TOKEN, 0, ZERO_ADDR, deadline, usdt_balance)
    # unwrapWETH9: WANKR → ANKR native ke wallet
